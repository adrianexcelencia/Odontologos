<template >
    ...
    <barra-navegacion></barra-navegacion>
      ...
      <div class="container-sm">
        ...
        border="top"
      >
       <b> Listado de campos</b><br>
  
       Mdificaciones de Datos 
      </v-alert>
          ...
          <!-- Tabla del Formulario -->
           <div class="row" >
              <div class="col  p-3 text-center ">
                ...
                <div class="col  p-3 text-center ">
                  <v-text-field v-model="buscar" label="Buscador nombre" append-inner-icon="mdi-magnify" variant="underlined" size="small" ></v-text-field>
                </div>
          </div>  
        </div>     
  
          
       <table :value="customers" class="table justify-center  darken-1 "  color="#FFFFFF"  >
          <thead>
            <tr>
              <th ><v-chip class="justify-center ">id_Field</v-chip></th>
              <th ><v-chip class="justify-center ">nombre</v-chip></th>
              <th ><v-chip class="justify-center ">Opocion</v-chip></th>
              
            </tr>
          </thead>
          <tbody v-for="Listaform of ListaFormularios" :key="Listaform.id_Field">
            <tr>
              <td class="bg-white " >
                
                  <v-icon start icon="mdi-arrow-right" ></v-icon>  {{ Listaform.id_Field }}
              ...
              <td class="nowrap bg-white " >
                <div class="row">
                  <div class="col-auto">
                     <v-btn @click="editar(Listaform)"  prepend-icon="mdi-open-in-new" class="sm"></v-btn>
                  </div>
      ...
      <v-form fast-fail @submit.prevent>
            ...
            <div class="row" >
              <div class="col  p-3 text-center ">
                <v-text-field  v-model="id_Field" label=" Identificador de campo" variant="underlined"></v-text-field>
               </div>
              <div class="col  p-3 text-center">
                <v-text-field v-model="nombre" label="Ingresar nombre" variant="underlined"></v-text-field>
              </div>
              <div class="col  p-3 text-center">
                <v-text-field v-model="Opcion" label="Nombre del metodo de Api" variant="underlined"></v-text-field>
               
              </div>
              <div class="col  p-3 text-center">
                <v-text-field v-model="Orden" label="Orden de aparicion en pantalla" variant="underlined"></v-text-field>
              </div>
        ...
        <div class="row" >
          ...
          <div class="col  p-6 text-center">
            <v-select
                  ...
                  :items="options"
                  label="Mostrar en grilla"
                  variant="underlined"
                  ></v-select>
          </div>
          <div class="col  p-6 text-center">
                <v-select
                  v-model="selectedItemPosi"
                  :items="optionsPosi"
                  label="Seleccionar posicion en pantalla"
                  variant="underlined"
                  ></v-select>
              </div>
          ...
          <v-btn  prepend-icon="mdi-cancel" color="#FF0000" v-on:click="mostrarConfirmacionEliminar(this.id_Field)" >   <v-spacer></v-spacer> Anular</v-btn>
          </div>
          <!--alerta suceso-->
        </v-form>
 <div class="col  p-3 text-center">
  ...
  <v-layout style=" max-width: 100%;">
    ...
    <v-card  class="px-4 py-2  text-center w-100" elevation="16">
  ...
  export default {
    ...
    methods: {
      ...
      editar(Listaform){
        ...
        this.Entrega = 1;
        this.dialog = true;
        this.id_Field = Listaform.id_Field;
        this.nombre = Listaform.nombre;
        this.Opcion = Listaform.opciones;
        this.selectedItemPosi = Listaform.posi;
        this.selectedItem = Listaform.ver;
        this.Orden = Listaform.orden;
      },
      async fetch() {
        
        this.MostrarSpinner = true; //abrir spinner mientras realiza la solicitud
        //`/api/ConfigForm/ListaFiel/${this.id_Field}`
        const respuesta = await this.axios.get("/api/ConfigForm/ListaField?pTipo="+this.$route.params.idConfigForm)
          .then((respuesta) => {
    ...
    this.idConfigFormToDelete = this.id_Field; // Guarda el idConfigForm en data
      ...
      async Grabar() {
        ...
        //alert("Agrega Campo");
          //await this.axios.post(`/api/ConfigForm/AgregarCampos/1/0/${this.nombre}/${this.value_list}/${this.mask_library}/${this.assumed_value}/${this.length}`)
          //Funciona Correcto
          await this.axios.post(`/api/ConfigForm/AgregarCampos/1/0/${this.nombre}/${this.value_list}/${this.mask_library}/${this.assumed_value}/${this.length}`)
          .then(datos => {
            this.mostrarAlertaEliminar = false;
      ...
      async Modificar() {
          ...
          await this.axios.put(`/api/ConfigForm/ModificaField/1/${this.id_Field}/${this.$route.params.idConfigForm}/${this.nombre}/${this.Opcion}/${this.Orden}/${this.selectedItem}/${this.selectedItemPosi}`)
          .then(datos => {
            this.mostrarAlertaEliminar = false;
      ...
      async eliminarform(idConfigFormToDelete) {
        ...
        await this.axios.put(`/api/ConfigForm/ModificaField/2/${this.idConfigFormToDelete}/${this.$route.params.idConfigForm}/0/0/0/0/0`)
