<template >
  ...
  <barra-navegacion></barra-navegacion>
    ...
    <div class="container-sm">
      ...
      border="top"
    >
     <b> Tipo Primitivo Boolean</b><br>

     Altas, bajas y modificaciones de Datos Boolean
    </v-alert>
    ...
    <div class="shadow p-3 mb-2 bg-white rounded table-container text-sm container-sm" v-if="$store.state.permisos.includes(11) & !NoHayRegistros">
     ...
     <table :value="customers" class="table justify-center bg-blue darken-1 "  color="#006699"  >
        <thead>
          <tr>
            <th ><v-chip class="justify-center primary">boolean_id</v-chip></th>
            <th ><v-chip class="justify-center primary">true_value</v-chip></th>
            <th ><v-chip class="justify-center primary">false_value</v-chip></th>
            <th ><v-chip class="justify-center primary">assumed_value</v-chip></th>
            <th  ><v-chip class="justify-center primary">Ver Datos</v-chip></th>
          </tr>
        </thead>
        <tbody v-for="Listaform of ListaFormularios" :key="Listaform.boolean_id">
          <tr>
            <td class="bg-white " >
              
                <div class="justify-center " color="primary"><b> » {{ Listaform.boolean_id }}</b></div>
           
          </td>
            <td class="bg-white ">{{ Listaform.true_value }}</td>
            <td  class="bg-white ">{{ Listaform.false_value }}</td>
            <td  class="bg-white ">
    ...
    <v-form fast-fail @submit.prevent>
          ...
          <div class="row" >
            <div class="col  p-3 text-center ">
              <v-text-field  v-model="boolean_id" label="Ingresar boolean_id" variant="underlined"></v-text-field>
             </div>
            <div class="col  p-3 text-center">
...
export default {
  ...
  methods: {
    ...
    editar(Listaform){
      ...
      this.Entrega = 1;
      this.dialog = true;
      this.boolean_id = Listaform.boolean_id;
      if(Listaform.true_value == 0) // -asignar valor al Check True
        this.true_value = false;
      else
        this.true_value = true;

      if(Listaform.false_value == 0) // -asignar valor al Check false
        this.false_value = false;
      else
        this.false_value = true;
    ...
    async fetch() {
      ...
      const respuesta = await this.axios.get("/api/ConfigForm/ListaCamposBoolean?pTipo=1")
        .then((respuesta) => {
  ...
  this.idConfigFormToDelete = this.boolean_id; // Guarda el idConfigForm en data
    ...
    async Grabar() {
        ...
        await this.axios.post(`/api/ConfigForm/AgregarCampos/4/0/${this.true_value}/${this.false_value}/${this.assumed_value}/n/0`)
        .then(datos => {
          this.mostrarAlertaEliminar = false;
    ...
    async eliminarform(idConfigFormToDelete) {
      await this.axios.put(`/api/ConfigForm/EliminaCampos/4/${this.idConfigFormToDelete}`)
