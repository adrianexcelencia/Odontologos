  ...
  <div class="row bg-white" >
    ...
    <div class="col-md-6  p-3 text-center ">
        ...
        <table :value="customers" class="table justify-center  darken-1 "  color="#FFFFFF"  >
          <thead>
            <tr>
              <th ><v-chip class="justify-center ">Codigo</v-chip></th>
          ...
          <tbody v-for="Listaform of rows1" :key="Listaform.codigo">
            <tr>
              <td class="bg-white " >
                  ...
                  <v-icon start icon="mdi-arrow-right" ></v-icon>  {{ Listaform.codigo }}
              ...
              <td class="nowrap bg-white " >
                <div class="row">
                  <div class="col-auto">
                     <v-btn @click="editar(Listaform)" color="#FF0000" prepend-icon="mdi-trash-can-outline" class="sm"></v-btn>
                  </div>
...
data.forEach(item => { IdTipo = item.codigo;   }); })
  .catch(err => { console.log(err); });
    ...
    await this.axios.post(`/api/ConfigForm/AgregarRelacionP?pTipo=1&pCodigo=${this.remitoValue}&pProveedor=${this.id}&pInstitucion=${IdTipo}`)
...
data.forEach(item => { IdTipo = item.codigo;   }); })
  .catch(err => { console.log(err); });
       ...
       const respuesta = await this.axios.get(`/api/ConfigForm/ListaProveedores?pTipo=1&pCodigo=0&pInstitucion=${IdTipo}`)
          .then((respuesta) => {
            //this.ListaFormulariosArticulos = respuesta.data.lista
            this.rows = respuesta.data.lista;
