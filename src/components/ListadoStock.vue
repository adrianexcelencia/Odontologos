<template>
   ...
   <!-- Alerta de confirmación personalizada -->
     ...
     <div>
       <v-alert
        ...
        border="top"
      >
       <b>   Listado de stock de articulos  </b> 
  ...
  <br>
   ...
   html2canvas(clonedTable, { scale: 2 }).then(canvas => {
     ...
     const title = "Listado de Stock"; // Reemplaza con tu título
     pdf.setFontSize(10);
  ...
  data.forEach(item => { IdTipo = item.codigo;   }); })
    .catch(err => { console.log(err); });
        ...
        const respuesta = await this.axios.get(`/api/ConfigForm/ListaArticulosStock?pTipo=${IdTipo}`)
           .then((respuesta) => {
             //this.ListaFormulariosArticulos = respuesta.data.lista
             this.rows = respuesta.data.lista;
   ...
   tbody, td, tfoot, th, thead, tr{
     ...
     thead{
         box-shadow: 0px 0px 9px 0px rgb(1, 83, 34);
         border-radius:2rem;
         height: 100%;
         align-items: center;
         width:100%;
