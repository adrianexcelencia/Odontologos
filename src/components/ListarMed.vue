 ...
 <!-- Alerta de confirmación personalizada -->
   ...
   <div>
     <v-alert
      ...
      border="top"
    >
     <b>   Listado de practicas autorizadas  </b> 
...
<br>
 <div class="row bg-white" >
             <div class="col-md-2">
               <v-btn @click="fetchArticulosMed" rounded="xl" class="bg-primary" block prepend-icon="mdi-autorenew" >Actualizar</v-btn>
               </div>
               <div class="col-md-2 ">
 ...
 html2canvas(clonedTable, { scale: 2 }).then(canvas => {
   ...
   const title = "Listado de Practicas";// + this.remitoValue; // Reemplaza con tu título
   pdf.setFontSize(10);
   ...
   pdf.save('Listado-Practicas.pdf');
   ...
   document.body.removeChild(tempDiv);
     ...
     XLSX.writeFile(workbook, "Listado-Practicas.xlsx");
   },
     async  MostrarDatos(){ 
       
       const respuesta = await this.axios.get(`/api/ConfigForm/ListarConsumoId?pTipo=${this.idUsuario}&pId=${this.remitoValue}`)
         .then((respuesta) => {
 ...
 //}
   ...
   async fetchArticulosMed() {
      ...
      const respuesta = await this.axios.get(`/api/ConsultarWs/ConsultaAutorizados?DniAfi=${this.remitoValue}&CodObra=000&Matri=${this.idUsuario}&pCoseguro=000`)
         .then((respuesta) => {
           //this.ListaFormulariosArticulos = respuesta.data.lista
